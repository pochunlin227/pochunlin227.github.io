import{s as l,d as h}from"./helpers-BwuIyHmI.js";const A={title:"02 Components/Form controls/Search result rows",parameters:h(['"Search stocks & ETFs" result rows — ticker chip, name, price, solid green/red change pill.',"","**States** default · hover/highlight (`surface.raised`). **Mobile** long fund names wrap to two lines; price + pill stay pinned right."].join(`
`))},a={render:()=>`
    <div class="sb-h">third row shows the hover/highlight state</div>
    <div style="max-width:520px">
      ${[["A","Agilent Technologies Inc.","S$142.92","+0.00%","pos",""],["AA","ALCOA CORP","S$32.10","-0.01%","neg",""],["AAON","AAON Inc.","S$95.51","+0.01%","pos","background:var(--arta-sys-color-surface-raised)"],["AAXJ","iShares MSCI All Country Asia ex Japan Index Fund","S$73.01","+0.00%","pos",""]].map(([n,t,c,i,o,p])=>`
        <div class="arta-ticker-row" style="${p}">
          <span class="arta-ticker-chip">${n}</span>
          <span class="arta-ticker-row__name">${t}</span>
          <span class="arta-ticker-row__price">${c}</span>
          <span class="arta-change arta-change--${o}">${i}</span>
        </div>`).join("")}
