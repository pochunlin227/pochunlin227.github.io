import{s as i,d as l}from"./helpers-BwuIyHmI.js";const d={title:"02 Components/Charts/Allocation treemap",parameters:l(["Micro Index review treemap. Tile weight is expressed through the grid template; **hover raises the tile and reveals the white % chip**.","","**Tokens** `arta.cmp.treemap.*`. **Mobile** re-grids to 2 columns."].join(`
`))},a={render:()=>`
    ...
    <div class="arta-treemap" style="max-width:640px;grid-template-columns:1.2fr 1.2fr 1.6fr;grid-auto-rows:96px">
      ${[["AAPL","15.00%"],["GOOG","15.00%"],["NVDA","21.00%"],["AMZN","15.00%"],["META","15.00%"],["MSFT","19.00%"]].map(([t,p])=>`
        <div class="arta-treemap__tile">${t}<span class="arta-treemap__chip">${p}</span></div>`).join("")}
    </div>`,parameters:i(`<div class="arta-treemap" style="grid-template-columns:1.2fr 1.2fr 1.6fr;grid-auto-rows:96px">
