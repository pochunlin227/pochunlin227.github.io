import{s as d,d as n}from"./helpers-BwuIyHmI.js";const c={title:"02 Components/Form controls/Radio rows",parameters:n(["Allocation-preset rows (AMP personalization). Selected row floods **violet** with a filled check dot — the loudest selection state in the product, reserved for one-of-N decisions that change money outcomes.","","**States** default · hover (stroke strengthens) · selected (violet fill + ✓). **Tokens** `arta.cmp.radiorow.*`. **Mobile** full-width rows, unchanged."].join(`
`))},o={render:()=>`
